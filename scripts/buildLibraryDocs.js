...
const processReadmeText = (text, publishName) => {
    ...
    return text.replace(reg, (main, img, imgPath) =>
         `${img}(${url.resolve(`https://github.com/TargetProcess/TP3MashupLibrary/raw/master/${publishName}/`,
            imgPath)})`
    );
};
...
const processReadme = (publishName) => {
    ...
    ShellString(processReadmeText(cat(file), publishName)).to('build/library/README.mkd');
